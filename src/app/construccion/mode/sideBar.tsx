...
export default function SideBar(
  { user, setStructure }: { user: User, setStructure: Dispatch<SetStateAction<number | null>> }

) {
  const [sideBar, setSideBar] = useState<boolean>(true)

  const SideBarIcon = ({ collector, user }: { collector: Collectors, user: User }) => {
    if(user.level >= collector.unlock_level) {
      return (
        <div className='sidebar-icon group' onClick={() => {setStructure(collector.id)}} >
          {collector.img}
      ...
      return(
        <div className='min-lev-req group '>
          ...
          <span className=' sidebar-name  group-hover:scale-100 opacity-80 '>
            {`You must be Level: ${collector.unlock_level} to unlock ${collector.name}`}
